...
public class WorldGenWitchsWoodTree extends WorldGenAbstractTree {
	...
	public boolean generate(World world, Random rand, BlockPos pos)  {
		...
		// add 8 to offset the generation. This should prevent the generation from 
		// spilling over into the next chunk and causing cascading world generation lag.
		x = pos.getX() /* + (8 * (pos.getX() / Math.abs(pos.getX())))*/;
		z = pos.getZ() /* + (8 * (pos.getZ() / Math.abs(pos.getZ())))*/;
		y = pos.getY();
		...
		if(!flag) {
			...
			boolean isSoil = state.getBlock().canSustainPlant(state, world, down, EnumFacing.UP, (BlockWitchsWoodSapling) BlockInit.WITCHS_WOOD_SAPLING);
